...
Those two examples illustrate the benefits of the pervasive use of ad hoc polymorphism and multiple dispatch in Julia: it allows for the construction of reusable abstractions. More explicitely, using those one could combine them using our reduced product implementation in a single value domain, for example $\text{Interval}_{\text{Int}}\cap\left(\sqcup^3\text{Const}\right)$. In Julia this would be \verb~Prod{Tuple{Interval{Int},FiniteSum{Const,3}}}~ and canonicalizes e.g.

\[ \left[0,3\right]\cap\left\{1,2,5\right\} = \left[1,2\right]\cap\left\{1,2\right\} \]

However, those two domains need tighter integration to realize that small intervals can describe a finite set of values.
We can easily add this behavior with a method definition such as

\begin{lstlisting}[language=julia]
meet_ext(v1::FiniteSum{Const,n}, v2::Interval{Int}) =
   v2.hi - v2.lo <= n ? meet(v1, map(Const, FiniteSum([v2.lo:v2.hi...])) : v1
\end{lstlisting}

Now, we also have

\[ \left[0,2\right]\cap\top = \left[0,2\right]\cap\left\{0,1,2\right\} \]
...
One can see it as the \emph{free} heap, in the sense of a free algebraic object.
Each node is uniquely identified by the field path that leads to it from the function argument.
...
The usefulness of this tool exceeded our -- admittedly low -- expectations. A screen capture is provided in Figure \ref{bla}.
